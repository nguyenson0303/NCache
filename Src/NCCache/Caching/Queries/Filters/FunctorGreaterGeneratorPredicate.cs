            ...
            if (store != null)
            {
                ...
                if (Inverse)
                    store.GetData(
                        ...
                        ComparisonType.LESS_THAN_EQUALS, queryContext.InternalQueryResult, mergeType, queryContext.CancellationToken);
                else
                    store.GetData(
                        generator.Evaluate(((MemberFunction) functor).MemberName, queryContext.AttributeValues),
                        ComparisonType.GREATER_THAN, queryContext.InternalQueryResult, mergeType, queryContext.CancellationToken);
            }
            else
            {
        ...
        internal override void Execute(QueryContext queryContext, Predicate nextPredicate)
        {
            ...
            if (store != null)
            {
                ...
                if (Inverse)
                    store.GetData(
                        generator.Evaluate(((MemberFunction) functor).MemberName, queryContext.AttributeValues),
                        ComparisonType.LESS_THAN_EQUALS, queryContext.InternalQueryResult, CollectionOperation.Union, queryContext.CancellationToken);
                else
                    store.GetData(
                        generator.Evaluate(((MemberFunction) functor).MemberName, queryContext.AttributeValues),
                        ComparisonType.GREATER_THAN, queryContext.InternalQueryResult, CollectionOperation.Union, queryContext.CancellationToken);
            }
            else 
            {
                throw new AttributeIndexNotDefined("Index is not defined for attribute '" +
